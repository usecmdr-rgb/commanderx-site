...
export default function AlohaIntelligence() {
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/aloha/ask", {
        method: "POST",
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-xl font-semibold">Ask Aloha</h3>
      </div>
      ...
      <form onSubmit={handleAsk} className="mb-4">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask about calls, sentiment, or contacts..."
            disabled={loading}
