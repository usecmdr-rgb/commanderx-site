...
export default function WorkflowManager() {
  ...
  const fetchWorkflows = async () => {
    ...
    try {
      const response = await fetch(`/api/beta/workflows?userId=${userId}`);
  ...
  const createWorkflow = async (workflow: Omit<WorkflowType, "id" | "createdAt" | "updatedAt">) => {
    try {
      const response = await fetch("/api/beta/workflows", {
        method: "POST",
  ...
  const deleteWorkflow = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/beta/workflows/${id}`, {
        method: "DELETE",
      });
  ...
  const executeWorkflow = async (workflowId: string) => {
    ...
    try {
      const response = await fetch("/api/beta/runWorkflow", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
